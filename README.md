# Aravind Vengarai

Welcome! I'm a recent graduate from Georgia Tech with a master's degree in Computer Science, embarking on my journey as a Software Engineer. Below is a collection of my academic accomplishments, work experiences, research, and projects that together paint a picture of my journey and expertise.

- [Education](#education)
- [Work Experience](#work-experience)
- [Research](#research)
- [Projects](#projects)
- [Connect](#connect)

## Education

**Georgia Institute of Technology** - Atlanta, GA  
- M.S. in Computer Science, 3.85/4.00 - Highest Honors (Aug 2019 – Dec 2023)  
- B.S. in Computer Science, 3.84/4.00 - Highest Honors (Aug 2019 – Dec 2023)

## Work Experience

### Lumen Technologies - Software Engineering Intern
Monroe, LA (May 2023 – Aug 2023)
- Enhanced automation for production support using GitHub Actions, reducing refresh time from 3 hours to 5 minutes and saving over $8,000 annually.
- Led UI test automation by integrating Playwright with Testkube, achieving a 70% reduction in test creation effort.
- Implemented a feature for suspending/resuming deployments, ensuring environment consistency during demos.

### NCR Corporation - Software Engineering Intern
Atlanta, GA (May 2022 – Aug 2022)
- Built Go-based REST APIs for a POS application, aiding 75% of user flows interacting with the UI.
- Devised an Apache Beam dataflow pipeline to transfer transaction logs to Big Query, improving query times by 10x.
- Automated compilation of dataflow templates to GCP storage buckets, streamlining deployments.

### Bashpole Software, Inc. - Software Engineering Intern
North Webster, IN (May 2021 – July 2021)
- Developed a JSP full-stack application, enhancing user engagement and conversion rates for thousands of viewers monthly.
- Decreased intern onboarding time by 67% by consolidating landing page projects into a single Maven project.

### Georgia Tech, College of Computing - Teaching Assistant
Atlanta, GA (Jan 2021 – Dec 2023)
- Educated over 400 students in data structures and algorithms, and over 200 in database systems.

## Research

### Undergraduate Research Assistant
Georgia Tech, College of Computing (Dec 2020 – May 2021)
- Developed an Android app for tracking PTSD patients' progress, utilized by 11 doctors from Emory University.
- Integrated services for app usage, geolocation, body movement, and audio data collection, improving patient treatment efficacy.

## Projects

### BuzzEx - Marketplace Delivery App
Developed a React Native app to add delivery functionality to Facebook Marketplace and Craigslist, enhancing user experience and sales efficiency.
- **[Demo](https://www.youtube.com/watch?v=bAAWD3mCmRQ)** | **[GitHub](https://github.com/AravindVengarai/buzzEx)**

### FarmSim - Java Farming Simulator
Created a Java-based game that simulates farm management, including strategic elements and handling unpredictable natural events.
- **[Game](https://drive.google.com/file/d/1dmjMEeIxskIK4vFdaMX1y77rbvE7u4Id/view)** | **[GitHub](https://github.com/AravindVengarai/FarmGame)**

### NBA Playoff Predictor
Employed machine learning techniques to predict NBA playoff outcomes using mid-season performance data.
- **[GitHub](https://kimayamcolaco.github.io/CS7641-Team-25/)**

### DiffPrep - Evaluating Transferability of Data Preprocessing Pipelines
Investigated the transferability of data preprocessing pipelines in machine learning tasks, improving data preparation efficiency.
- **[Paper](https://drive.google.com/file/d/1z8VgEsyKtG1HST7_e-BNn2rQT1QgKE4F/view)** | **[GitHub](https://github.com/AravindVengarai/DiffPrep)**

### Minimum Vertex Cover Problem
Analyzed algorithms for the Minimum Vertex Cover Problem in graph theory, emphasizing the balance between speed and solution quality.
- **[Paper](https://drive.google.com/file/d/11Hp4_ue1MzMnjoaARkMggKBd8X4Of8IX/view)** | **[GitHub](https://github.com/AravindVengarai/DiffPrep)**

### Kerberos POS - ID Verification App for POS Systems
Developed an app for POS systems to automate the ID verification process, significantly enhancing transaction efficiency.
- **[Screens](https://drive.google.com/file/d/1kfEaINtPV8QG9mH9IS7UCTEH9CFVIgPl/view)** | **[GitHub](https://github.com/AravindVengarai/DiffPrep)**

## Connect

Feel free to [connect with me on LinkedIn](https://www.linkedin.com/in/aravindvengarai/) or check out [my GitHub](https://github.com/aravindvengarai) for more on my projects and experiences.
